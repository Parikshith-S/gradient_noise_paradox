...
def train_hybrid_epoch(model, shadow_model, optimizer, train_loader, privacy_engine, epoch):
    model.train()
    criterion = nn.CrossEntropyLoss()
    losses = []
    
    # Define Attacker on the SHADOW model
    atk = torchattacks.PGD(shadow_model, eps=config.ATTACK_EPS, alpha=config.ATTACK_ALPHA, steps=config.ATTACK_STEPS)
    ...
    for i, (images, labels) in enumerate(train_loader):
        ...
        if i % 50 == 0:
            ...
            print(f"Epoch {epoch} | Batch {i} | Loss: {loss.item():.4f} | ε: {epsilon:.2f}")
...
def evaluate(model, shadow_model, test_loader):
    ...
    atk = torchattacks.PGD(shadow_model, eps=config.ATTACK_EPS, alpha=config.ATTACK_ALPHA, steps=10)
    ...
    for images, labels in test_loader:
        ...
        total += labels.size(0)
        if total > 500: break # Speed up for demo
